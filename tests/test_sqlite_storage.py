...
class TestSQLiteStorage:
    ...
    def test_multiple_days_data(self, temp_db, sample_user_data):
        ...
        with patch('src.sqlite_storage.datetime') as mock_datetime:
            ...
            mock_datetime.now.return_value.isoformat.return_value = "2025-08-10T10:00:00"
            storage.save_daily_data(sample_user_data)
            ...
            mock_datetime.now.return_value.isoformat.return_value = "2025-08-11T10:00:00"
    ...
    def test_get_weekly_progress(self, temp_db, sample_user_data):
        ...
        for i in range(10):  # More than 7 days
            with patch('src.sqlite_storage.datetime') as mock_datetime:
                ...
                mock_datetime.now.return_value.isoformat.return_value = f"{date}T10:00:00"
                storage.save_daily_data(sample_user_data)
    ...
    def test_cleanup_old_data(self, temp_db, sample_user_data):
        ...
        with patch('src.sqlite_storage.datetime') as mock_datetime:
            ...
            mock_datetime.now.return_value.isoformat.return_value = "2025-06-01T10:00:00"
            storage.save_daily_data(sample_user_data)
            
            # Recent data (should be kept)
            mock_datetime.now.return_value.strftime.return_value = "2025-08-14"
            mock_datetime.now.return_value.isoformat.return_value = "2025-08-14T10:00:00"
            storage.save_daily_data(sample_user_data)
        ...
        with patch('src.sqlite_storage.SQLiteStorage.cleanup_old_data') as mock_cleanup:
            # Mock the cleanup to actually delete old data
            import sqlite3
            with sqlite3.connect(temp_db) as conn:
...
class TestStorageMigration:
    ...
    def test_sqlite_to_json_migration(self, temp_db_path, temp_json_dir, sample_json_data):
