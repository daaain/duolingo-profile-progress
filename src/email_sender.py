...
def send_email(report, email_config, subject_prefix=""):
    ...
    try:
        if not all([
            ...
            print("❌ Email configuration incomplete. Please check environment variables or config file.")
            return False
        
        msg = MIMEMultipart()
        msg['From'] = email_config['sender_email']
        msg['To'] = ', '.join(email_config['family_email_list'])
        msg['Subject'] = f"Duolingo Family League - {subject_prefix}{datetime.now().strftime('%B %d, %Y')}"
        ...
        server.sendmail(
            ...
            email_config['family_email_list'],
            text
        )
        server.quit()
