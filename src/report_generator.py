    ...
    for member_name, data in results.items():
        if 'error' not in data:
            leaderboard_data.append({
                ...
                'target_xp': data.get('total_languages_xp', data.get('target_languages_xp', 0)),
                'data': data
            })
...
def generate_daily_report(results):
    ...
    for i, member in enumerate(leaderboard[:3], 1):
        ...
        report.append(f"{emoji} {member['name']}: {member['streak']} day streak | {member['weekly_xp']} weekly XP")
...
def generate_weekly_report(results, goals):
    ...
    for member_name, data in results.items():
        ...
        if data.get('language_progress'):
            ...
            report.append(f"   📚 Active Languages: {', '.join(data['active_languages'])}")
