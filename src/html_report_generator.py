...
def _generate_leaderboard(results: dict[str, Any]) -> list[dict[str, Any]]:
...
def generate_daily_html_report(results: dict[str, Any]) -> str:
    ...
    i18n = get_i18n()
    leaderboard = _generate_leaderboard(results)
    ...
    for i, member in enumerate(leaderboard[:3], 1):
        emoji = position_emojis[i - 1] if i <= 3 else f"{i}."
        ...
        if streak_count == 1:
            streak_text = i18n.get("day_streak", count=streak_count)
        else:
            streak_text = i18n.get("days_streak", count=streak_count)
    ...
    for member_name, data in results.items():
        if "error" not in data and data["streak"] == 0:
            alerts.append(f"<li>• {member_name} {i18n.get('needs_to_practice')}</li>")
    
    if alerts:
        streak_alerts_html = f'<div class="alert warning"><ul>{"".join(alerts)}</ul></div>'
    else:
...
def generate_weekly_html_report(results: dict[str, Any], goals: dict[str, Any]) -> str:
    ...
    for member_name, data in results.items():
        if "error" in data:
            member_details.append(f"""
                <div class="member-detail">
                    <div class="member-header">
                        <div class="member-avatar">👤</div>
                        <div>
                            <div class="member-title">{member_name}</div>
                        </div>
                    </div>
                    <div class="progress-item">
                        ❌ {i18n.get("unable_to_check", error=data['error'])}
                    </div>
                </div>
            """.strip())
            continue
        ...
        if data.get("language_progress"):
            ...
            for lang, progress in data["language_progress"].items():
                if progress["xp"] > 0:
                    ...
                    language_items.append(f"""
                        <div class="language-item">
                            <span class="language-name">{lang}:</span> {i18n.get("not_started_yet")}
                        </div>
                    """.strip())
        ...
        member_details.append(f"""
            <div class="member-detail">
                <div class="member-header">
                    ...
                    <div>
                        <div class="member-title">{member_name}</div>
                        <div class="member-username">({data['username']})</div>
                    </div>
                </div>
